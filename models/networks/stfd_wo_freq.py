...
class STWOFreqModel(torch.nn.Module):
    ...
    def forward(self, x, adj):
        ...
        x_t = torch.cat([
            self.diffusion.q_sample(x, t), # batch_size x seq x node_size x 1
            self.diffusion.q_sample(x, t), # batch_size x seq x node_size x 1
            self.diffusion.q_sample(x, t), # batch_size x seq x node_size x 1
        ], dim=-1) # batch_size x seq x node_size x dim_feature

        y = torch.cat([
            x, # batch_size x seq x node_size x 1
            x, # batch_size x seq x node_size x 1
            x, # batch_size x seq x node_size x 1
        ], dim=-1) # batch_size x seq x node_size x dim_feature
        ...
        tgt_hat = self.st_transformer(t=src, src=src, adj=adj, tgt=tgt_input, tgt_mask=tgt_mask)
