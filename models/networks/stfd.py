...
class STDiffusionModel(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 只是用来补充不同小波基实验的，这块之后要删除掉，db2
        # self.fc_l  = torch.nn.Linear(6, dim_seq)
        # self.fc_h1 = torch.nn.Linear(6, dim_seq)
        # self.fc_h2 = torch.nn.Linear(10, dim_seq)
        # self.fc_h3 = torch.nn.Linear(17, dim_seq)

        # 只是用来补充不同小波基实验的，这块之后要删除掉，db4
        # self.fc_l  = torch.nn.Linear(10, dim_seq)
        # self.fc_h1 = torch.nn.Linear(10, dim_seq)
        # self.fc_h2 = torch.nn.Linear(13, dim_seq)
        # self.fc_h3 = torch.nn.Linear(19, dim_seq)
    ...
    def forward(self, x, adj):
        ...
        # 将 t_l 置零抹去 x_l 作为 condition的影响，后续删除
        # time_embedding = time_embedding + t_l.zero_()
    ...
    def freq_trans(self, x):
        ...
        x_l = signals[0]
        x_t_h1 = self.diffusion.q_sample(signals[1], t)
        x_t_h2 = self.diffusion.q_sample(signals[2], t)
        x_t_h3 = self.diffusion.q_sample(signals[3], t)
